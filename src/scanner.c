...
bool try_match_tag(Scanner *scanner, TSLexer *lexer, char *open_tag) {
	unsigned open_tag_len = strlen(open_tag);
	for (unsigned i = 0; i < open_tag_len; i++) {
		if (lexer->lookahead != open_tag[i]) {
			return false;
		}
...
bool scan_tag_close(
	...
	if (!valid_symbols[HTML_TAG_CLOSE]) {
		return false;
	}
	...
	do {
		...
		if (try_match_tag(scanner, lexer, open_tag)) {
			enum TokenType result_symbol = is_option_tag(open_tag)
				? HTML_OPTION_TAG_CLOSE
				: HTML_TAG_CLOSE;
...
unsigned tree_sitter_spml_external_scanner_serialize(
	...
	for (unsigned i = 0; i < scanner->open_tag_count; i++) {
		name_length = strlen(scanner->open_tags[i]);
